...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # RELAY FUNCTION
        self.ui.relay1.stateChanged.connect(lambda: self.turn_on(
            1, 1 if self.ui.relay1.isChecked() else 0))
        self.ui.relay2.stateChanged.connect(lambda: self.turn_on(
            2, 2 if self.ui.relay2.isChecked() else 0))
        self.ui.relay3.stateChanged.connect(lambda: self.turn_on(
            3, 3 if self.ui.relay3.isChecked() else 0))
        self.ui.relay4.stateChanged.connect(lambda: self.turn_on(
            4, 4 if self.ui.relay4.isChecked() else 0))
    ...
    def readSetting(self):
        ...
        if not os.path.exists(settings_file) or os.stat(settings_file).st_size == 0:
            with open('AppSettings.ini', 'w') as configfile:
                ...
                config['User'] = {"port": "COM1", "note1": "Note1:",
                                  "note2": "Note3:", "note3": "Note3:", "note4": "Note4:"}
                config['Modified'] = {
                    'User: ': '', 'datetime': '%s' % datetime.datetime.now()}
                config.write(configfile)
        config.read('AppSettings.ini')
        try:
    ...
    def Changed(self, event):
        ...
        config.read('AppSettings.ini')
        with open('AppSettings.ini', 'w') as configfile:
            config['User'] = {"port": "%s" % self.port, "note1": "%s" % self.ui.note1.text(
            ), "note2": "%s" % self.ui.note2.text(), "note3": "%s" % self.ui.note3.text(), "note4": "%s" % self.ui.note4.text()}
